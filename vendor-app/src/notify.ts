...
export async function registerPushToken(app: "vendor") {
  ...
  const endpoint = "/vendors/push-token";
  ...
  try {
    ...
    // Handle 401 errors more gracefully - vendor might not be authenticated yet
    if (error?.response?.status === 401) {
      console.warn(
        `Push token registration failed for ${app}: Vendor not authenticated (401)`
      );
...
export function attachNotificationListeners(
  ...
  const responseSub = Notifications.addNotificationResponseReceivedListener(
    (response) => {
      ...
      if (data?.type === "new_order") navigate("OrderDetails", { orderId: data.orderId });
    }
  );
