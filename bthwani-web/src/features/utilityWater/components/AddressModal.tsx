// src/features/utilityWater/components/AddressModal.tsx
      ...
      <Box sx={{ maxHeight: 400, overflow: "auto", p: 2 }}>
        {addresses.map((address, index) => (
          <Grow in key={address._id} timeout={300 + index * 100}>
            <Button
              variant={selectedId === address._id ? "contained" : "outlined"}
              onClick={() => onSelect(address._id)}
              startIcon={
                <Avatar
                  sx={{
                    bgcolor:
                      selectedId === address._id ? "white" : "primary.main",
                    width: 24,
                    height: 24,
                  }}
                >
                  <Place
                    sx={{
                      fontSize: 14,
                      color:
                        selectedId === address._id ? "primary.main" : "white",
                    }}
                  />
                </Avatar>
              }
