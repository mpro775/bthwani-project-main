...
export default function ResetVerify() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await verifyResetCode({ emailOrPhone: contact, code });
      setOk(true);
    } catch (err: unknown) {
  ...
  return (
    <Box
      sx={{
        ...
        background: `linear-gradient(135deg, ${theme.palette.warning.light}15 0%, ${theme.palette.info.light}10 100%)`,
        position: 'relative',
        overflow: 'hidden',
        '&::before': {
          content: '""',
          ...
          <Card
            sx={{
              ...
              '&::before': {
                ...
                right: 0,
                bottom: 0,
                background: `linear-gradient(135deg, ${theme.palette.warning.light}08 0%, ${theme.palette.info.light}05 100%)`,
                borderRadius: 4,
                zIndex: 1,
              }
            }}
                  ...
                  <Box>
                    <Typography
                      ...
                      sx={{
                        ...
                        mb: 1,
                        fontSize: '2.2rem',
                        background: 'linear-gradient(45deg, #FF9800 30%, #2196F3 90%)',
                        backgroundClip: 'text',
              ...
              {!ok ? (
                <Fade in timeout={1000}>
                  <Box component="form" onSubmit={submit} sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                    <TextField
                      ...
                      sx={{
                        '& .MuiInputBase-root': {
                          ...
                          border: contact ? `2px solid ${theme.palette.warning.main}` : '1px solid',
                          borderColor: contact ? 'warning.main' : 'divider',
                          boxShadow: contact ? `0 0 0 3px ${theme.palette.warning.light}30` : 'none',
                    ...
                    <Button
                      type="submit"
                      disabled={loading || !contact.trim() || code.length < 6}
                      variant="contained"
                      fullWidth
                      size="large"
                      sx={{
