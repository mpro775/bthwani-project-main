...
import { KenzConversation, KenzChatListResponse } from "@/api/kenzChatApi";
import { getConversations } from "@/api/kenzChatApi";
import { useAuth } from "@/auth/AuthContext";
import COLORS from "@/constants/colors";

type NavigationProp = NativeStackNavigationProp<RootStackParamList, "KenzChatList">;

const KenzChatListScreen = () => {
  const navigation = useNavigation<NavigationProp>();
  const { user } = useAuth();
  const [conversations, setConversations] = useState<KenzConversation[]>([]);
  ...
  const loadConversations = useCallback(async (cursor?: string, isLoadMore = false) => {
    try {
      ...
      const response: KenzChatListResponse = await getConversations(cursor, 25);
  ...
  const handleConversationPress = useCallback((conversation: KenzConversation) => {
    navigation.navigate("KenzChat", { conversationId: conversation._id });
  }, [navigation]);
  ...
  const getUnreadCount = (conversation: KenzConversation) => {
    if (!user) return 0;
  ...
  const getOtherUser = (conversation: KenzConversation) => {
    if (!user) return null;
    if (conversation.ownerId._id === user.uid) {
  ...
  const renderConversationItem = ({ item }: { item: KenzConversation }) => {
...
export default KenzChatListScreen;
