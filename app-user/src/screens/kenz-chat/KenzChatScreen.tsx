...
import { KenzMessage, getConversation, getMessages, sendMessage, markAsRead } from "@/api/kenzChatApi";
import { connectKenzChatSocket } from "@/hooks/useKenzChatSocket";
import { useAuth } from "@/auth/AuthContext";
import { refreshIdToken } from "@/api/authService";
import COLORS from "@/constants/colors";

type RouteProps = RouteProp<RootStackParamList, "KenzChat">;
type NavigationProp = NativeStackNavigationProp<RootStackParamList, "KenzChat">;

const KenzChatScreen = () => {
  ...
  const socketRef = useRef<ReturnType<typeof connectKenzChatSocket> | null>(null);
  ...
  useEffect(() => {
    ...
    const setupSocket = async () => {
      try {
        ...
        const socket = connectKenzChatSocket(
          token,
          conversationId,
          (newMessage) => {
            setMessages(prev => {
  ...
  const renderMessage = ({ item }: { item: KenzMessage }) => {
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerInfo}>
          <Text style={styles.headerTitle}>
            {conversation.kenzId?.title || "إعلان"}
          </Text>
