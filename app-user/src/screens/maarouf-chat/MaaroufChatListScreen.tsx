...
import { MaaroufConversation, MaaroufChatListResponse } from "@/api/maaroufChatApi";
import { getConversations } from "@/api/maaroufChatApi";
import { useAuth } from "@/auth/AuthContext";
import COLORS from "@/constants/colors";

type NavigationProp = NativeStackNavigationProp<RootStackParamList, "MaaroufChatList">;

const MaaroufChatListScreen = () => {
  const navigation = useNavigation<NavigationProp>();
  const { user } = useAuth();
  const [conversations, setConversations] = useState<MaaroufConversation[]>([]);
  ...
  const loadConversations = useCallback(async (cursor?: string, isLoadMore = false) => {
    try {
      ...
      const response: MaaroufChatListResponse = await getConversations(cursor, 25);
  ...
  const handleConversationPress = useCallback((conversation: MaaroufConversation) => {
    navigation.navigate("MaaroufChat", { conversationId: conversation._id });
  }, [navigation]);
  ...
  const getUnreadCount = (conversation: MaaroufConversation) => {
    if (!user) return 0;
  ...
  const getOtherUser = (conversation: MaaroufConversation) => {
    if (!user) return null;
    if (conversation.ownerId._id === user.uid) {
  ...
  const renderConversationItem = ({ item }: { item: MaaroufConversation }) => {
...
export default MaaroufChatListScreen;
