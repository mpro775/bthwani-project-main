...
import { MaaroufConversation, ConversationStatus } from './entities/maarouf-conversation.entity';
import { MaaroufMessage } from './entities/maarouf-message.entity';
import { CreateConversationDto } from './dto/create-conversation.dto';
import { SendMessageDto } from './dto/send-message.dto';

@Injectable()
export class MaaroufChatService {
  constructor(
    @InjectModel(MaaroufConversation.name)
    private conversationModel: Model<MaaroufConversation>,
    @InjectModel(MaaroufMessage.name)
    private messageModel: Model<MaaroufMessage>,
    @InjectModel('Maarouf')
    private maaroufModel: Model<any>,
  ) {}

  async createConversation(dto: CreateConversationDto, interestedUserId: string) {
    const maaroufId = new Types.ObjectId(dto.maaroufId);
    
    // التحقق من وجود الإعلان
    const maarouf = await this.maaroufModel.findById(maaroufId);
    if (!maarouf) {
      throw new NotFoundException({
        code: 'MAAROUF_NOT_FOUND',
        userMessage: 'الإعلان غير موجود',
      });
    }

    const ownerId = new Types.ObjectId(maarouf.ownerId.toString());
