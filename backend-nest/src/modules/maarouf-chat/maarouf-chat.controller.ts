...
@ApiTags('معروف — المحادثات')
@ApiBearerAuth()
@Controller('maarouf-chat')
@UseGuards(UnifiedAuthGuard)
export class MaaroufChatController {
  constructor(private readonly maaroufChatService: MaaroufChatService) {}

  @Post('conversations')
  @ApiOperation({ summary: 'إنشاء محادثة جديدة', description: 'إنشاء محادثة جديدة لإعلان معروف' })
  ...
  async createConversation(
    ...
    return this.maaroufChatService.createConversation(dto, userId);
  }
  ...
  async getConversations(
    ...
    return this.maaroufChatService.getConversations(userId, cursor, limitNum);
  }
  ...
  async getConversationById(
    ...
    const userId = req.user._id || req.user.id || '';
    return this.maaroufChatService.getConversationById(id, userId);
  }
  ...
  async sendMessage(
    ...
    return this.maaroufChatService.sendMessage(conversationId, senderId, dto);
  }
  ...
  async getMessages(
    ...
    const userId = req?.user._id || req?.user.id || '';
    const limitNum = limit ? parseInt(limit, 10) : 25;
    return this.maaroufChatService.getMessages(conversationId, userId, cursor, limitNum);
  }
  ...
  async markAsRead(
    ...
    const userId = req.user._id || req.user.id || '';
    return this.maaroufChatService.markAsRead(conversationId, userId);
  }
  ...
  async getUnreadCount(@Req() req: AuthenticatedRequest) {
    const userId = req.user._id || req.user.id || '';
    return this.maaroufChatService.getUnreadCount(userId);
  }
}
