...
export class KenzChatGateway
  implements OnGatewayConnection, OnGatewayDisconnect
{
  @WebSocketServer()
  server: Server;

  private logger = new Logger('KenzChatGateway');
  ...
  constructor(
    ...
    private kenzChatService: KenzChatService,
  ) {}
  ...
  async handleConnection(client: Socket) {
    try {
      ...
      if (!token) {
        this.logger.warn(
          `KenzChat connection rejected: No token - ${client.id}`,
        );
        client.emit('error', {
          code: 'NO_TOKEN',
      ...
      this.logger.log(
        `User connected to KenzChat: ${payload.sub} - Socket: ${client.id}`,
      );
      ...
      this.logger.error(
        `KenzChat authentication failed for socket ${client.id}:`,
  ...
  handleDisconnect(client: Socket) {
    ...
    this.logger.log(`Client disconnected from KenzChat: ${client.id} (${userId})`);
  ...
  @SubscribeMessage('kenz-chat:join')
  async handleJoin(
    ...
    try {
      ...
      // التحقق من الصلاحيات
      await this.kenzChatService.getConversationById(data.conversationId, userId);
  ...
  @SubscribeMessage('kenz-chat:leave')
  async handleLeave(
    @ConnectedSocket() client: Socket,
    @MessageBody() data: { conversationId: string },
  ) {
  ...
  @SubscribeMessage('kenz-chat:message')
  async handleMessage(
    @ConnectedSocket() client: Socket,
    ...
    try {
      ...
      const message = await this.kenzChatService.sendMessage(
        data.conversationId,
        userId,
        dto,
      );
      ...
      const conversation = await this.kenzChatService.getConversationById(
        data.conversationId,
        userId,
      );
      ...
      const room = `conversation_${data.conversationId}`;
      this.server.to(room).emit('kenz-chat:message:new', {
        ...message.toObject(),
        conversationId: data.conversationId,
      });
      ...
      if (!this.connectedUsers.has(otherUserId)) {
        this.server.to(`user_${otherUserId}`).emit('kenz-chat:notification', {
          conversationId: data.conversationId,
  ...
  @SubscribeMessage('kenz-chat:typing')
  async handleTyping(
    @ConnectedSocket() client: Socket,
    ...
    if (data.isTyping) {
      // إرسال مؤشر الكتابة
      client.to(room).emit('kenz-chat:typing', {
        conversationId: data.conversationId,
        userId,
        isTyping: true,
      });
  ...
  @SubscribeMessage('kenz-chat:read')
  async handleRead(
    @ConnectedSocket() client: Socket,
    @MessageBody() data: { conversationId: string },
  ) {
    try {
      ...
      await this.kenzChatService.markAsRead(data.conversationId, userId);

      const room = `conversation_${data.conversationId}`;
      this.server.to(room).emit('kenz-chat:read', {
        conversationId: data.conversationId,
        userId,
      });
