...
@WebSocketGateway({
  ...
  namespace: '/kawader-chat',
})
export class KawaderChatGateway
  implements OnGatewayConnection, OnGatewayDisconnect
{
  @WebSocketServer()
  server: Server;

  private logger = new Logger('KawaderChatGateway');
  ...
  constructor(
    ...
    private kawaderChatService: KawaderChatService,
  ) {}
  ...
  async handleConnection(client: Socket) {
    try {
      ...
      if (!token) {
        this.logger.warn(
          `KawaderChat connection rejected: No token - ${client.id}`,
        );
        client.emit('error', {
          code: 'NO_TOKEN',
      ...
      this.logger.log(
        `User connected to KawaderChat: ${payload.sub} - Socket: ${client.id}`,
      );
      ...
      this.logger.error(
        `KawaderChat authentication failed for socket ${client.id}:`,
  ...
  handleDisconnect(client: Socket) {
    ...
    this.logger.log(`Client disconnected from KawaderChat: ${client.id} (${userId})`);
  ...
  @SubscribeMessage('kawader-chat:join')
  async handleJoin(
    ...
    try {
      ...
      // التحقق من الصلاحيات
      await this.kawaderChatService.getConversationById(data.conversationId, userId);
      ...
      client.emit('kawader-chat:joined', {
  ...
  @SubscribeMessage('kawader-chat:leave')
  async handleLeave(
    @ConnectedSocket() client: Socket,
    @MessageBody() data: { conversationId: string },
  ) {
  ...
  @SubscribeMessage('kawader-chat:message')
  async handleMessage(
    @ConnectedSocket() client: Socket,
    ...
    try {
      ...
      const message = await this.kawaderChatService.sendMessage(
        data.conversationId,
        userId,
        dto,
      );
      ...
      const conversation = await this.kawaderChatService.getConversationById(
        data.conversationId,
        userId,
      );
      ...
      const room = `conversation_${data.conversationId}`;
      this.server.to(room).emit('kawader-chat:message:new', {
        ...message.toObject(),
        conversationId: data.conversationId,
      });
      ...
      if (!this.connectedUsers.has(otherUserId)) {
        this.server.to(`user_${otherUserId}`).emit('kawader-chat:notification', {
          conversationId: data.conversationId,
  ...
  @SubscribeMessage('kawader-chat:typing')
  async handleTyping(
    @ConnectedSocket() client: Socket,
    ...
    if (data.isTyping) {
      // إرسال مؤشر الكتابة
      client.to(room).emit('kawader-chat:typing', {
        conversationId: data.conversationId,
        userId,
        isTyping: true,
      });
  ...
  @SubscribeMessage('kawader-chat:read')
  async handleRead(
    @ConnectedSocket() client: Socket,
    @MessageBody() data: { conversationId: string },
  ) {
    try {
      ...
      await this.kawaderChatService.markAsRead(data.conversationId, userId);

      const room = `conversation_${data.conversationId}`;
      this.server.to(room).emit('kawader-chat:read', {
        conversationId: data.conversationId,
        userId,
      });
