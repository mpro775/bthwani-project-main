...
import { KawaderConversation, ConversationStatus } from './entities/kawader-conversation.entity';
import { KawaderMessage } from './entities/kawader-message.entity';
import { CreateConversationDto } from './dto/create-conversation.dto';
import { SendMessageDto } from './dto/send-message.dto';

@Injectable()
export class KawaderChatService {
  constructor(
    @InjectModel(KawaderConversation.name)
    private conversationModel: Model<KawaderConversation>,
    @InjectModel(KawaderMessage.name)
    private messageModel: Model<KawaderMessage>,
    @InjectModel('Kawader')
    private kawaderModel: Model<any>,
  ) {}

  async createConversation(dto: CreateConversationDto, interestedUserId: string) {
    const kawaderId = new Types.ObjectId(dto.kawaderId);
    
    // التحقق من وجود الإعلان
    const kawader = await this.kawaderModel.findById(kawaderId);
    if (!kawader) {
      throw new NotFoundException({
        code: 'KAWADER_NOT_FOUND',
        userMessage: 'الإعلان غير موجود',
      });
    }

    const ownerId = new Types.ObjectId(kawader.ownerId.toString());
  ...
  async getConversationById(conversationId: string, userId: string) {
    const conversation = await this.conversationModel
      ...
      .populate('kawaderId', 'title budget scope status description metadata')
      .populate('ownerId', 'name email phone')
      .populate('interestedUserId', 'name email phone')
      .exec();
