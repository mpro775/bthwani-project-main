...
export default function WithdrawScreen() {
  ...
  const handleWithdraw = async () => {
    ...
    if (!selectedMethodObj) {
      Alert.alert("خطأ", "يرجى اختيار طريقة سحب");
      return;
    }

    if (parseFloat(amount) < selectedMethodObj.minAmount) {
      Alert.alert(
        "خطأ",
        `الحد الأدنى للسحب هو ${selectedMethodObj.minAmount} ريال`
      );
      return;
    }

    if (parseFloat(amount) > selectedMethodObj.maxAmount) {
      Alert.alert(
        "خطأ",
        `الحد الأقصى للسحب هو ${selectedMethodObj.maxAmount} ريال`
      );
      return;
    }

    let accountInfoData: Record<string, unknown> = {};
    ...
    if (selectedMethod === "bank_transfer") {
      ...
      if (!accountInfo.agentCode) {
        Alert.alert("خطأ", "يرجى إدخال رمز الوكيل");
        return;
      }
      accountInfoData = {
      ...
      <View style={styles.content}>
        ...
        <View style={styles.section}>
            ...
            {selectedMethod &&
              methods.find((m) => m.id === selectedMethod) && (
                <Text style={styles.hintText}>
                  الحد الأدنى:{" "}
                  {methods.find((m) => m.id === selectedMethod)?.minAmount}{" "}
                  ريال | الحد الأقصى:{" "}
                  {methods.find((m) => m.id === selectedMethod)?.maxAmount}{" "}
                  ريال
                </Text>
              )}
          </View>
          ...
          {selectedMethod === "agent" && (
            <View style={styles.inputGroup}>
              <Text style={styles.label}>رمز الوكيل *</Text>
              <TextInput
                style={styles.input}
                value={accountInfo.agentCode}
                onChangeText={(text) =>
                  setAccountInfo({ ...accountInfo, agentCode: text })
                }
                keyboardType="numeric"
