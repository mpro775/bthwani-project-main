...
import { getAvailableAmaniOrders, getMyAmaniOrders, AmaniOrder } from '../../api/amani';
...
export default function AmaniListScreen() {
  ...
  const loadAvailableOrders = useCallback(async () => {
    try {
      ...
      Alert.alert('خطأ', 'فشل في تحميل الطلبات المتاحة');
    }
  }, []);
  ...
  const loadMyOrders = useCallback(async () => {
    try {
      ...
      Alert.alert('خطأ', 'فشل في تحميل طلباتي');
    }
  }, []);
      ...
      Alert.alert(
        ...
        [
          { text: 'إلغاء', style: 'cancel' },
          {
            ...
            onPress: async () => {
              try {
                await getAvailableAmaniOrders(); // This will auto-assign
                Alert.alert('نجح', 'تم قبول الطلب بنجاح');
                loadData();
              } catch (error: any) {
                Alert.alert('خطأ', error?.response?.data?.message || 'فشل في قبول الطلب');
              }
            },
          },
        ]
